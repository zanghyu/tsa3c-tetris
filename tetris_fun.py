...
class GameState:
    def __init__(self):
        ...
        pygame.display.set_caption('ts_model')
    ...
    def frame_step(self,input):
        ...
        if self.fallingPiece == None:
            ...
            if not self.isValidPosition():
                ...
                terminal = True
                reward = -10
                self.drawFallingPieceBoard()
                self.drawPiece(self.fallingPiece)
                pygame.display.update()
                falling_img = pygame.surfarray.array3d(pygame.display.get_surface())
                falling_img_x_t = cv2.cvtColor(cv2.resize(falling_img, STATE_SHAPE), cv2.COLOR_BGR2GRAY)
                ret, falling_img_x_t = cv2.threshold(falling_img_x_t, 1, 255, cv2.THRESH_BINARY)
                self.reinit()

                return x_t, falling_img_x_t, reward, terminal, cleared # can't fit a new piece on the self.board, so game over
        ...
        if falling_img_x_t is None:
            falling_img_x_t = x_t


        return x_t, falling_img_x_t, reward, terminal, cleared
    ...
    ## add
    def drawFallingPieceBoard(self):
        # draw the border around the self.board
        pygame.draw.rect(DISPLAYSURF, BORDERCOLOR, (XMARGIN - 3, TOPMARGIN - 7, (BOARDWIDTH * BOXSIZE) + 8, (BOARDHEIGHT * BOXSIZE) + 8), 5)
